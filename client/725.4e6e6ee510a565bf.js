...
let MembersComponent = /*#__PURE__*/(() => {
  class MembersComponent {
    ...
    GetMember() {
      ...
      this.organizationService.GetOrganizationMember(this.organizationDetails['_id'], this.start, this.limit).subscribe(response => {
        if (response && response.ReturnCode == 200 && response.err == 0) {
          ...
          if (this.membersListResponse && this.membersListResponse['Data'].list.length > 0) {
            // this.membersListResponse['Data'].forEach(element => {
    ...
    GetInvites() {
      this.masterService.showSpppiner();
      this.invitesListArray = [];
      this.organizationService.GetOrganizationInvites(this.organizationDetails['_id']).subscribe(response => {
    ...
    DeleteInvites(event) {
      ...
      confirmationDialog.afterClosed().subscribe(result => {
        if (result == true) {
          this.organizationService.DeleteInvitesMember(event.organization_id, data).subscribe(response => {
            if (response && response.ReturnCode == 200) {
              this.GetInvites();
    ...
    ResendInvites(event) {
      ...
      this.masterService.showSpppiner();
      this.organizationService.ResendInvites(this.organizationDetails['_id'], data).subscribe(response => {
        if (response && response.ReturnCode == 200) {
          this.masterService.hideSppiner();
          this.GetInvites();
          this.masterService.showSuccess(response.ReturnMsg, 'Success');
...
function TeamsComponent_div_7_Template(rf, ctx) {
  if (rf & 1) {
    ...
    _angular_core__WEBPACK_IMPORTED_MODULE_6__["ɵɵlistener"]("click", function TeamsComponent_div_7_Template_span_click_5_listener() {
      ...
      return _angular_core__WEBPACK_IMPORTED_MODULE_6__["ɵɵresetView"](ctx_r7.addNewTeam());
    });
  ...
  class TeamsComponent {
    ...
    removeTeam(element) {
      ...
      this.peopleService.RemoveTeam(this.organizationDetails['_id'], teamId).subscribe(response => {
        if (response.ReturnCode == 200 && response.err == 0) {
          this.GetTeams();
          this.masterService.showSuccess(response.ReturnMsg, "Success");
        }
      });
